...
def parse_args():
    ...
    parser.add_argument("--local", action="store_true",
                      help="Run in local-file mode (choose CSV or XML and use keyword scoring)")
    parser.add_argument("--mapping-file", type=str, default=None,
                      help="Path to a JSON file containing subsystem->keywords mapping")
    return parser.parse_args()


def select_local_file() -> Optional[str]:
    """Open a file dialog and return selected file path (CSV or XML)."""
    root = tk.Tk()
    root.withdraw()
...
def prompt_for_keywords() -> List[str]:
    """Prompt the user to enter comma-separated keywords via dialog."""
    root = tk.Tk()
    root.withdraw()
    kw_string = simpledialog.askstring("Keywords", "Enter comma-separated keywords to score against:")
    root.destroy()
    if not kw_string:
        return []
...
def prompt_for_subsystem_mapping() -> Dict[str, List[str]]:
    """Prompt the user to enter subsystem: keyword1, keyword2 lines.

    Example input (multi-line):
      Detection: radar, lidar
      AI/Fusion: neural, model, fusion
    Returns mapping dict: { 'Detection': ['radar','lidar'], ... }
    """
    root = tk.Tk()
    root.withdraw()
...
def edit_mapping_dialog(initial_mapping: Dict[str, List[str]]) -> Dict[str, List[str]]:
    """Show a multi-line edit dialog pre-filled with initial mapping and return updated mapping.

    The dialog content format matches prompt_for_subsystem_mapping (one subsystem per line).
    """
    text = ''
...
def save_mapping_to_file(mapping: Dict[str, List[str]], path: str) -> None:
    """Save mapping dict to a JSON file."""
    with open(path, 'w', encoding='utf-8') as f:
        json.dump(mapping, f, indent=2)


def load_mapping_from_file(path: str) -> Dict[str, List[str]]:
    """Load mapping dict from a JSON file."""
...
def read_local_file(file_path: str) -> List[Dict]:
    """Read CSV or XML file and return list of records with keys: id, patent_id, title, abstract.
    Supports CSV with headers 'Patent ID','Title','Abstract' or XML with elements containing those tags.
    """
    records = []
    p = Path(file_path)
...
def keyword_score(title: str, abstract: str, keywords: List[str] = None, mapping: Dict[str, List[str]] = None) -> Dict:
    """Keyword scoring that supports subsystem mapping.

    If 'mapping' is provided, counts matches per subsystem and includes matching
    subsystem names in the result. If mapping is None, falls back to simple
    keyword list counting with empty subsystem.

    Returns dict: { 'Relevance': 'High'|'Medium'|'Low', 'Subsystem': [..] }
    """
    text = f"{title} {abstract}".lower()
    # If mapping provided, compute subsystem matches
    subsystems = []
    total_matches = 0
    if mapping:
        ...
        if keywords:
            total_matches = sum(text.count(k) for k in keywords)
...
def main():
    ...
    try:
        ...
        # Local-file mode: open dialogs, read file, run keyword scoring
        if args.local:
            file_path = select_local_file()
            if not file_path:
                logger.info("No file selected, exiting local mode")
                return

            mapping = {}
            # Load mapping from file if provided via CLI
            if args.mapping_file:
                try:
                    mapping = load_mapping_from_file(args.mapping_file)
                    logger.info(f"Loaded mapping from {args.mapping_file}")
                except Exception as e:
                ...
                if not mapping:
                    logger.info("No subsystem mapping provided, exiting local mode")
                    return
            # Allow user to review/edit the mapping before scoring
            try:
                mapping = edit_mapping_dialog(mapping)
            except Exception:
                # Ignore dialog errors and continue with current mapping
                pass
                # Ask user if they want to save the mapping
                root = tk.Tk()
                root.withdraw()
                save_choice = simpledialog.askstring("Save mapping?", "Save mapping to file? Enter path or leave blank:")
                root.destroy()
                if save_choice:
                    try:
                        save_mapping_to_file(mapping, save_choice)
                        logger.info(f"Saved mapping to {save_choice}")
                    except Exception as e:
                        logger.error(f"Failed to save mapping: {e}")

            records = read_local_file(file_path)
            logger.info(f"Loaded {len(records)} records from {file_path}")

            results = []
            for i, p in enumerate(records, 1):
                patent_id = p.get('patent_id') or f'local-{i}'
                title = p.get('title', '')
                abstract = p.get('abstract', '')
                sha1 = compute_sha1(patent_id, abstract)

                logger.info(f"Scoring local record {i}/{len(records)}: {patent_id}")
                ...
                if cached:
                    ...
                    score = keyword_score(title, abstract, mapping=mapping)
                    relevance = score['Relevance']
                    subsystem = score['Subsystem']
                    store_result(conn, patent_id, sha1, relevance, subsystem, title, abstract, 'local-keyword')
                ...
                results.append({
                    ...
                    'abstract': abstract,
                    'relevance': relevance,
                    'subsystem': ';'.join(subsystem) if subsystem else ''
                })

            # Export results next to input file
            out_csv = Path(file_path).with_suffix('.scored.csv')
            ...
            logger.info(f"Local scoring complete. Results written to {out_csv}")
            return

        # Default Airtable/OpenAI flow
        while True:
            ...
            for i, p in enumerate(patents, 1):
                ...
                try:
                    ...
                    if relevance == "Low":
                        ...
                        logger.info(f"Will update Airtable record for {patent_id} with relevance: {relevance} and subsystems: {subsystem}")
